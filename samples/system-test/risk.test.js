...
test(`should handle numerical risk analysis errors`, async t => {
  const output = await tools.runAsync(
    `${cmd} numerical ${dataset} nonexistent ${numericField} ${topicName} ${subscriptionName}`,
    cwd
  );
...
test(`should handle categorical risk analysis errors`, async t => {
  const output = await tools.runAsync(
    `${cmd} categorical ${dataset} nonexistent ${uniqueField} ${topicName} ${subscriptionName}`,
    cwd
  );
...
test(`should handle k-anonymity analysis errors`, async t => {
  const output = await tools.runAsync(
    `${cmd} kAnonymity ${dataset} nonexistent ${topicName} ${subscriptionName} ${numericField}`,
    cwd
  );
...
test(`should handle k-map analysis errors`, async t => {
  const output = await tools.runAsync(
    `${cmd} kMap ${dataset} nonexistent ${topicName} ${subscriptionName} ${numericField} -t AGE`,
    cwd
  );
...
test(`should check that numbers of quasi-ids and info types are equal`, async t => {
  const errors = await tools.runAsyncWithIO(
    `${cmd} kMap ${dataset} nonexistent ${topicName} ${subscriptionName} ${numericField} -t AGE GENDER`,
    cwd
  );
  t.regex(errors.stderr, /Number of infoTypes and number of quasi-identifiers must be equal!/);
});
...
test(`should handle l-diversity analysis errors`, async t => {
  const output = await tools.runAsync(
    `${cmd} lDiversity ${dataset} nonexistent ${topicName} ${subscriptionName} ${numericField}`,
    cwd
  );
